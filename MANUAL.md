# mirstat Operational Manual

This document details the operational mechanics, configuration, disaster recovery, and troubleshooting procedures for a mirstat node and wallet.

## 1. Directory Structure and Configuration

mirstat separates node data from wallet data to ensure that running a public node does not expose private keys.
...
### Wallet Directory (`~/.mirstat/` by default)
...
```bash
mirstat wallet restore --path wallet.dat
```
...
```bash
mirstat wallet import-rewards --coinbase-file ./data/coinbase_seeds.jsonl --data-dir ./data
```
...
```bash
mirstat wallet export --path wallet.dat --coin <COIN_ID_HEX>
```

To rebuild a single coin from raw data:
```bash
mirstat wallet import --path wallet.dat --seed <SEED_HEX> --value <INTEGER> --salt <SALT_HEX>
```
...
```bash
mirstat node --mine --threads 3 --verify-threads 2
```

### Archival vs Pruned Mode

mirstat nodes can run in two modes regarding historical data:
...
* **Dust Sweeping:** If you accumulate too many siblings (exceeding the `MAX_TX_INPUTS` limit of 256), you must sweep them into a reusable MSS address using the consolidate command:
  ```bash
  mirstat wallet consolidate --address <WOTS_ADDRESS>
  ```
...
mirstat employs an ultra-fast $O(\log N)$ periodic health check. If the node loses power abruptly and the `Redb` state accumulator falls out of sync with the block headers, the node will log a `CRITICAL` error and initiate a **Self-Healing Rollback**. It will automatically rewind the chain state to the last safe `snapshot` (taken every 100 blocks) and replay the blocks to repair the database without requiring a full resync from Genesis.
...
```bash
mirstat wallet send --to <ADDRESS>:15 --private
```

### Auto-Shatter & Drip Mixing (`automix`)

For maximum anonymity, mirstat includes an automated dark-pool routing command. `automix` takes a large UTXO, shatters it into standard power-of-2 denominations, and stochastically "drips" them into active P2P CoinJoin pools over time with random delays, breaking temporal heuristics.

```bash
mirstat wallet automix --coin <COIN_ID>
```
...
## 5. mirstat Axe & Hardware Operations

mirstat is engineered to run on bare-metal ARM hardware (e.g., Raspberry Pi Zero 2 W).
