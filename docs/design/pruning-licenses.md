...
mirstat is deliberately designed for low-resource hardware. As the chain lengthens, the marginal cost of maintaining full history grows while the private benefit to any individual node declines. This creates a classic **tragedy of the commons**: too few archival nodes, making bootstrapping expensive or impossible for new participants, and slowly degrading the network's resilience.
...
- It respects the pure UTXO + covenant model mirstat already has (no oracles, no global state in the VM).
...
## 3. License Representation — Native mirstat Primitive
...
**Verdict from codebase analysis**: This can be deployed on the live mirstat network today with **zero consensus changes**. The v5 opcodes (`OP_READ_INPUT_STATE`, `OP_READ_OUTPUT_STATE`, `OP_THIS_ADDRESS`, `OP_OUTPUT_ADDRESS`, `OP_SUM_TO_ADDR`, `OP_MUL`, `OP_DIV`) make the required checks trivial.
...
This design re-uses mirstat's existing light-client and MMR infrastructure with almost no new cryptography.
...
**Rationale (from review)**: mirstat has no on-chain DAO or treasury. Any attempt to route a "tax" to a protocol-controlled address creates insurmountable custody and governance questions.
...
## 7. Direct Mapping to Existing mirstat Codebase (Implementation Readiness)
...
> This is a brilliant application of free-market economics to P2P network health. By tying the right to prune to a tradable covenant, you allow nodes with high bandwidth but low storage (like a VPS) to subsidize nodes with high storage but lower bandwidth (like a home NAS), keeping the mirstat network healthy, decentralized, and economically balanced.

The v4 design deliberately removes the last points of friction that would have required on-chain governance or global state, leaving only cryptography, voluntary trade, and local reputation — the ingredients mirstat already handles elegantly.
...
- **Startup wiring** ([src/main.rs](/home/nick/mirstat/mirstat/src/main.rs), [src/node.rs](/home/nick/mirstat/mirstat/src/node.rs)): `Node::register_my_licenses` + config + `--license-wallet` + `mirstat_LICENSE_WALLET_PASSWORD` env integration. Archival nodes now automatically load their held licenses from wallet at startup and respond to LicenseChallenge MMR Gossip Challenges.
...
This delivers a working, philosophically aligned, review-hardened foundation for the Prune Market using only mirstat's existing primitives (MMR, ChatV2, Confidential State Threads, Bayesian LightPeerState extension, covenants).
