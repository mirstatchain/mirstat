...
/// Serves the shared mirstat stylesheet.
///
/// # Reasoning
/// The dashboard links to `/mirstat.css` rather than embedding its own palette so it
...
/// so this resolves to the single canonical `src/rpc/mirstat.css` (the temporary copy
...
///
/// # Architecture
/// This server is designed to run independently from the core mirstat node. 
...
///
/// # Arguments
/// * `pool_address` - The mirstat address where the pool fee will be sent.
...
/// * `node_rpc_url` - The HTTP URL of the backend mirstat node (e.g., `http://10.0.0.5:8545`).
...
pub async fn run_stratum_pool(pool_address: String, bind_addr: String, node_rpc_url: String, pool_fee_percent: f64) {
    ...
    tokio::spawn(async move {
        let app = Router::new()
            ...
            .route("/mirstat.css", get(pool_css))   
    ...
    tokio::spawn(async move {
        ...
        // that matches the NEW counter and gets validated against the new mirstat,
            ...
            if (current_tip != last_network_tip || force_new_job) && !current_tip.is_empty() {
                ...
                *state_clone.current_tree.write().await = tree.clone();
                ...
                if let Some(template) = template_data {
                    if let Some(m_hex) = template["mining_mirstat"].as_str() {
