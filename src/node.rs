...
use crate::network::{Message, mirstatNetwork, NetworkEvent, MAX_GETBATCHES_COUNT, MAX_GETHEADERS_COUNT};
...
// and WebRTC paths drifted — the WebRTC path returned `post_tx_mirstat` as
// `mining_mirstat` while the HTTP path returned `compute_header_hash(&header)`,
...
///
/// The miner must grind nonces against the returned `mining_mirstat`, which
...
fn build_template_prefix(
    ...
    let prev_mirstat = state.mirstat;
...
fn finish_template(
    ...
    let prev_mirstat    = prefix.prev_mirstat;
    ...
    // ── Fold coinbase into mirstat, then absorb state_root ──────────────

    for cb in &coinbase {
        candidate.mirstat = hash_concat(&candidate.mirstat, &cb.coin_id());
    ...
    candidate.mirstat = hash_concat(&candidate.mirstat, &state_root);
    ...
    // post_tx_mirstat. verify_extension recomputes from the header hash on
    ...
    let candidate_header = BatchHeader {
        ...
        prev_header_hash,
        prev_mirstat,
        post_tx_mirstat: candidate.mirstat,
    ...
    Ok(crate::rpc::types::BlockTemplateResponse {
        mining_mirstat: hex::encode(mining_hash),
...
pub async fn new(
        ...
        let network = mirstatNetwork::new(keypair, listen_addr, bootstrap_peers, banned_peers).await?;
...
async fn process_verified_batches_chunk(
       ...
       if current_cursor < peer_height {
        ...
        if candidate_state.depth > self.state.depth
            || (candidate_state.depth == self.state.depth && candidate_state.mirstat < self.state.mirstat)
        {
            tracing::info!(
        ...
        match msg {
            ...
            Message::GetState => {
                let response = Message::StateInfo {
                    ...
                    mirstat: self.state.mirstat,
                };
                self.send_response(channel, response);
            }

            Message::StateInfo { height, depth, mirstat } => {
                self.ack(channel);
                ...
                if is_syncing {
                    ...
                    if is_sync_peer {
                        ...
                        if mirstat == self.state.mirstat && height == self.state.height {
                    ...
                    if depth > self.state.depth || height > self.state.height
                        || (depth == self.state.depth && mirstat < self.state.mirstat)
                    {
                ...
                if let Some(s) = self.sync.session.as_ref() {
                    if let SyncPhase::Headers { accumulated, .. } = &s.phase {
                        if let Some(last_acc) = accumulated.last() {
                            if headers[0].prev_header_hash != last_acc.extension.final_hash || headers[0].prev_mirstat != last_acc.post_tx_mirstat {
        ...
        tokio::spawn(async move {
            ...
            if is_valid {
                if let Some(snap) = &snapshot {
                    if all_headers.is_empty() || all_headers[0].prev_mirstat != snap.mirstat || all_headers[0].prev_header_hash != snap.header_hash {
                        tracing::warn!("Snapshot mirstat mismatch! Peer sent fraudulent snapshot. Aborting sync.");
                        is_valid = false;
        ...
        tokio::task::spawn_blocking(move || {
            ...
            for batch in &batches {
                ...
                if batch.extension.final_hash != header.extension.final_hash {
                    ...
                    if legacy_calc.post_tx_mirstat != header.post_tx_mirstat {
...
// ADAPTIVE DANDELION++: Dynamic fluff probability based on network size.
    ...
    async fn evaluate_alternative_chain(
        ...
        for (i, batch) in alternative_batches.iter().enumerate() {
            ...
            // FIX: Push `header_hash` instead of `mirstat`. 
            ...
            // Storing `mirstat` here breaks duplicate block detection after a reorg!
            new_history.push((height, candidate_state.header_hash, batch.clone()));
    ...
    /// Verifies that the in-memory UTXO set, mirstat, and disk records are perfectly aligned.
    ...
    async fn handle_new_batch(&mut self, batch: Batch, from: Option<PeerId>) -> Result<()> {
        ...
        if prev_header_hash != self.state.header_hash {
            ...
            if list.len() < 4 {
                ...
                tracing::warn!("Too many UNIQUE competing orphans for mirstat, dropping to prevent RAM exhaustion.");
            }
        ...
        match apply_batch(&mut candidate_state, &batch, self.recent_headers.make_contiguous(), &mut wots_oracle) {
            Ok(_) => {
                ...
                let is_reorg = best.height == self.state.height &&
                               best.mirstat != self.state.mirstat;
...
async fn try_apply_orphans(&mut self) {
        ...
        while let Some(mut batches) = self.orphan_batches.remove(&self.state.header_hash) {
            ...
            if !matched {
                break; // All candidates for this mirstat failed validation
            }
        }
    ...
    fn spawn_mining_task(&mut self) -> Result<()> {
        ...
        let pre_mine_mirstat = self.state.mirstat;
        let mut candidate_state = self.state.clone();
        ...
        for cb in &coinbase {
            ...
            candidate_state.mirstat = hash_concat(&candidate_state.mirstat, &coin_id);
        }
        ...
        candidate_state.mirstat = hash_concat(&candidate_state.mirstat, &state_root);
        // ---------------------------------
        ...
        let candidate_header = BatchHeader {
            ...
            prev_mirstat: pre_mine_mirstat,
            post_tx_mirstat: candidate_state.mirstat,
            extension: Extension { nonce: 0, final_hash: [0u8; 32] },
            timestamp: actual_timestamp,
    ...
    ///     pre  batch?.prev_mirstat = mirstat
    ///
    ...
    async fn handle_mined_batch(&mut self, batch: Batch) -> Result<()> {
        ...
        if self.state.mirstat != batch.prev_mirstat {
...
mod tests {
    ...
    async fn test_find_fork_point_logic() {
        ...
        let mirstat_0 = genesis_batch.extension.final_hash;

        // Create Batch 1
        let ext1 = create_extension(mirstat_0, 100);
        let mirstat_1 = ext1.final_hash;
        let batch1 = Batch {
            prev_mirstat: mirstat_0,
        ...
        // Create Batch 2
        let ext2 = create_extension(mirstat_1, 200);
        let mirstat_2 = ext2.final_hash;
        let batch2 = Batch {
            prev_mirstat: mirstat_1,
        ...
        node.state.height = 2;
        node.state.mirstat = mirstat_2;

        // ---------------------------------------------------------
        // Case A: Linear Extension (No Fork)
        // ---------------------------------------------------------
        // Remote sends Batch 3, which builds on Batch 2.
        let ext3 = create_extension(mirstat_2, 300);
        let batch3 = Batch {
            prev_mirstat: mirstat_2,
        ...
        // Since batch3.prev_mirstat == node.mirstat (mirstat_2),
        ...
        // prev_mirstat = mirstat_1.
        let ext2_prime = create_extension(mirstat_1, 999); // Different nonce -> different hash
        ...
        let ext1_prime = create_extension(mirstat_0, 555);
    ...
    async fn print_genesis_values_for_the_website() {
        ...
        println!("\n  initial_mirstat : {}", hex::encode(State::genesis().0.mirstat));
        println!("  genesis block    : {}", hex::encode(g.extension.final_hash));
        println!("  height-1 mirstat: {}\n", hex::encode(node.state.mirstat));
    }
    ...
    fn find_genesis_nonce() {
        ...
        for nonce in 0u64.. {
            let ext = create_extension(mining_mirstat, nonce);
...
mod complex_tests {
    ...
    /// 1. Simulates transaction application to calculate the correct `mirstat`.
    ...
    fn make_valid_batch(prev_state: &State, timestamp_offset: u64, transactions: Vec<Transaction>) -> Batch {
        ...
        let mining_seed = prev_state.mirstat; 
       ...
       // 3. Compute State Root
        ...
        let candidate_header = BatchHeader {
            ...
            prev_mirstat: prev_state.mirstat,
            post_tx_mirstat: candidate_state.mirstat,
            extension: Extension { nonce: 0, final_hash: [0u8; 32] },
            timestamp,
            target,
    ...
    async fn reorg_restores_mempool_transactions() {
        ...
        // the *exact* genesis batch saved by the node to ensure mirstats align.
        let genesis_batch = node.storage.load_batch(0).unwrap().unwrap();
    ...
    async fn crash_recovery_preserves_state() {
        ...
        let coins_before;
        {
            ...
            mirstat_before = node.state.mirstat;
        ...
        assert_eq!(node2.state.mirstat, mirstat_before, "mirstat must survive restart");
    ...
    async fn fresh_node_syncs_full_chain() {
        ...
        assert_eq!(fresh.state.mirstat, miner.state.mirstat);
    ...
    async fn partial_sync_completes() {
        ...
        assert_eq!(behind.state.mirstat, miner.state.mirstat);
    }
    ...
    async fn concurrent_miners_reorg() {
        ...
        assert_eq!(node.state.mirstat, chain_b_state.mirstat, "must adopt chain B's mirstat");
    }
...
fn build_divergent_chain(
    ...
    /// prev_mirstat linkage — the "Frankenstein chain" bug.
    #[tokio::test]
    async fn aborted_sync_does_not_corrupt_disk() {
        ...
        assert_eq!(node.state.mirstat, pre_sync_state.mirstat);
    }
    ...
    fn block_template_mining_hash_matches_consensus() {
        // The invariant the original bug violated: the mining_mirstat handed to
        ...
        // Replay the coinbase folding to recover post_tx_mirstat the same way
        // build_block_template_inner did.
        let v2 = crate::core::types::is_v2_at(state.height);
        let mut post_tx_mirstat = state.mirstat;
        let mut temp_coins = state.coins.clone();
        for cb in &batch.coinbase {
            let cid = cb.coin_id();
            post_tx_mirstat = hash_concat(&post_tx_mirstat, &cid);
            temp_coins.insert(cid, v2);
        }
        let smt_root   = hash_concat(&temp_coins.root(v2), &state.commitments.root(v2));
        let state_root = hash_concat(&smt_root, &state.chain_mmr.root(v2));
        post_tx_mirstat = hash_concat(&post_tx_mirstat, &state_root);
        ...
        assert_eq!(hex::encode(consensus_hash), resp.mining_mirstat,
            "mining_mirstat diverged from compute_header_hash — the original bug is back");
    }
