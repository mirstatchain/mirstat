...
use mirstat::*;
use mirstat::compute_address;
use mirstat::wallet::{self, Wallet};
use mirstat::core::wots;
use mirstat::core::state::apply_batch;
use mirstat::network::{mirstatNetwork, NetworkEvent, Message};
use mirstat::core::types; // for hash + count_leading_zeros in PoAW generator
...
struct Config {
    ...
    /// Password must be supplied via the mirstat_LICENSE_WALLET_PASSWORD environment variable.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl Config {
    ...
    fn create_default(path: &std::path::Path) -> Result<()> {
        ...
        let default_contents = r#"# mirstat node configuration
            #
...
enum Command {
    Node {
        ...
        /// Requires mirstat_LICENSE_WALLET_PASSWORD in the environment.
        #[arg(long)]
...
enum WalletAction {
    ...
    IssueLicense {
        ...
        /// Fixed royalty fee (in mirstat) paid to the original issuer on every transfer.
...
fn read_password(prompt: &str) -> Result<Vec<u8>> {
    if let Ok(val) = std::env::var("mirstat_PASSWORD") {
        if val.is_empty() { anyhow::bail!("mirstat_PASSWORD is set but empty"); }
...
fn read_password_confirm() -> Result<Vec<u8>> {
    if std::env::var("mirstat_PASSWORD").is_ok() {
...
fn parse_output_spec(s: &str) -> Result<ParsedOutput> {
    ...
    let addr = mirstat::core::types::parse_address_flexible(parts[0])
    ...
    tracing_subscriber::registry()
        .with(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "mirstat=info,tower_http=debug".into()),
        )
    ...
    match cli.command {
        Command::Node { data_dir, port, rpc_port, rpc_bind, peer, mine, threads,
                verify_threads, listen, config, prune, license_wallet, backend } => {
                    mirstat::core::gpu_mining::set_backend(match backend.to_ascii_lowercase().as_str() {
                        "gpu"  => mirstat::core::gpu_mining::Backend::Gpu,
                        "cpu"  => mirstat::core::gpu_mining::Backend::Cpu,
                        "auto" => mirstat::core::gpu_mining::Backend::Auto,
                        other  => {
                            tracing::warn!("unknown --backend '{other}', using auto");
                            mirstat::core::gpu_mining::Backend::Auto
                        }
                    });
        ...
        Command::Pool { pool_address, bind_addr, rpc_port, rpc_host, fee } => {
            #[cfg(not(target_arch = "wasm32"))]
            {
                ...
                mirstat::pool::run_stratum_pool(pool_address, bind_addr, node_rpc_url, fee).await;
                Ok(())
            }
        ...
        Command::Miner { pool_url, payout_address, worker, threads } => {
            #[cfg(not(target_arch = "wasm32"))]
            {
                ...
                let stats = std::sync::Arc::new(std::sync::RwLock::new(mirstat::mining::StratumStats::default()));
                ...
                mirstat::mining::run_stratum_client(pool_url, payout_address, worker, threads, hash_counter, stats).await;
                Ok(())
            }
            #[cfg(target_arch = "wasm32")]
            {
...
async fn filter_scan(
    ...
    use mirstat::core::filter::match_any;
...
async fn targeted_scan(
    ...
    for &height in heights {
        ...
        let batch: mirstat::core::Batch = resp.json().await?;
        ...
        for tx in &batch.transactions {
            let outputs = match tx {
                mirstat::core::Transaction::Reveal { outputs, .. }
                | mirstat::core::Transaction::Consolidate { outputs, .. } => outputs,
                _ => continue,
            };
            for out in outputs {
...
async fn wallet_spend_script(
    ...
    for cref in &coin_refs {
        ...
        let zeros = match client.get(&pow_url).send().await {
            Ok(r) if r.status().is_success() => r
                ...
                .await
                .map(|p| p.zeros)
                .unwrap_or(mirstat::rpc::pow_governor::MIN_ZEROS),
            _ => mirstat::rpc::pow_governor::MIN_ZEROS,
        };
        let timestamp = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)?
            .as_secs();
        // Height is 0 for /search — same preimage the server rebuilds.
        let (nonce, hash) = mirstat::rpc::pow_governor::solve_pow(cref, 0, timestamp, zeros);
        ...
        for res in search_data.results {
            if res.result_type == "output_coin_id" || res.result_type == "coinbase_coin_id" {
                ...
                let batch: mirstat::core::Batch = block_resp.json().await?;
                
                if res.result_type == "coinbase_coin_id" {
                    for cb in &batch.coinbase {
                        if hex::encode(cb.coin_id()) == *cref {
                            spending_coins.push(mirstat::wallet::WalletCoin {
                    ...
                    if let Some(tx) = batch.transactions.get(tx_idx) {
                        match tx {
                            mirstat::core::Transaction::Reveal { outputs, .. } | 
                            mirstat::core::Transaction::Consolidate { outputs, .. } => {
                                for out in outputs {
    ...
    let script_address = mirstat::core::types::hash(&bytecode);
    ...
    for arg in &to_args {
        ...
        match parse_output_spec(arg)? {
            ParsedOutput::Standard(addr, val) => {
                outputs.push(mirstat::core::OutputData::Standard { address: addr, value: val, salt });
                out_sum += val;
            }
            ParsedOutput::Stateful(addr, state) => {
                outputs.push(mirstat::core::OutputData::Confidential { address: addr, commitment: state, salt });
            }
        }
    }
    ...
    if let Some(burn_str) = burn_data {
        ...
        outputs.push(mirstat::core::OutputData::DataBurn { payload, value_burned: val });
        out_sum += val;
    }
    ...
    if in_sum > out_sum + base_fee {
        ...
        for denom in mirstat::core::decompose_value(change_val) {
            let seed = wallet.allocate_next_wots_seed()?;
            let pk = mirstat::core::wots::keygen(&seed);
            let addr = mirstat::core::compute_address(&pk);
            let salt: [u8; 32] = rand::random();
            let idx = outputs.len();
            outputs.push(mirstat::core::OutputData::Standard { address: addr, value: denom, salt });
    ...
    let commitment = mirstat::core::compute_commitment(&input_coin_ids, &output_commit_hashes, &tx_salt);
    ...
    for coin_id in &input_coin_ids {
        ...
        let input_bytecode = if coin.address == script_address {
            ...
            hex::encode(mirstat::core::script::compile_p2pk(&coin.owner_pk))
        };
    ...
    let reveal_req = rpc::SendTransactionRequest {
        ...
        outputs: outputs.iter().map(|o| match o {
            mirstat::core::OutputData::Standard { address, value, salt } => rpc::OutputDataJson::Standard {
                address: hex::encode(address),
                value: *value,
                salt: hex::encode(salt),
            },
            mirstat::core::OutputData::Confidential { address, commitment, salt } => rpc::OutputDataJson::Confidential {
                address: hex::encode(address),
                commitment: hex::encode(commitment),
                salt: hex::encode(salt),
            },
            mirstat::core::OutputData::DataBurn { payload, value_burned } => rpc::OutputDataJson::DataBurn {
    ...
    for (idx, seed) in change_seeds {
        if let mirstat::core::OutputData::Standard { address, value, salt } = &outputs[idx] {
            let owner_pk = mirstat::core::wots::keygen(&seed);
            wallet.data.coins.push(mirstat::wallet::WalletCoin {
...
fn wallet_abandon(path: &PathBuf, address_hex: String) -> Result<()> {
    ...
    let addr = mirstat::core::types::parse_address_flexible(&address_hex)
        .map_err(|e| anyhow::anyhow!(e))?;
...
async fn wallet_restore(path: &PathBuf, phrase_arg: Option<String>, rpc_port: u16, rpc_host: String) -> Result<()> {
    ...
    mirstat::wallet::hd::master_seed_from_mnemonic(&phrase)?;
    ...
    println!("\nCRITICAL WARNING: Run `mirstat wallet scan` before sending any transactions.");
...
fn wallet_compile(path: &std::path::Path) -> Result<()> {
    ...
    match mirstat::core::script::assemble(&source) {
        Ok(bytecode) => {
            let address = mirstat::core::types::hash(&bytecode);
...
fn wallet_receive(path: &PathBuf, label: Option<String>) -> Result<()> {
    ...
    println!("  {}\n", mirstat::core::types::encode_address_with_checksum(&address));
...
fn wallet_generate(path: &PathBuf, count: usize, label: Option<String>) -> Result<()> {
    ...
    for i in 0..count {
        ...
        println!("  [{}] {}", wallet.keys().len() - 1, mirstat::core::types::encode_address_with_checksum(&pk));
...
async fn wallet_list(path: &PathBuf, rpc_port: u16, rpc_host: String, full: bool, live: bool, search: Option<String>, filter_type: Option<String>) -> Result<()> {
    ...
    for mss in wallet.mss_keys() {
        mss_addrs.insert(mirstat::compute_address(&mss.master_pk));
    }

    if wallet.coin_count() > 0 || !wallet.list_licenses().is_empty() {
        let mut coins_by_addr: std::collections::HashMap<[u8; 32], Vec<(usize, &mirstat::wallet::WalletCoin, Result<bool, ()>)>> = std::collections::HashMap::new();
        ...
        for addr in sorted_addrs {
            ...
            let addr_str = mirstat::core::types::encode_address_with_checksum(&addr);
    ...
    if !wallet.keys().is_empty() {
        ...
        for (i, k) in wallet.keys().iter().enumerate() {
            let display = if full { mirstat::core::types::encode_address_with_checksum(&k.address) } else { hex::encode(&k.address) };
...
async fn wallet_consolidate(
    ...
    let target_addr = mirstat::core::types::parse_address_flexible(&address_str)
        .map_err(|e| anyhow::anyhow!(e))?;
    ...
    if live_coins.len() > mirstat::core::MAX_CONSOLIDATE_INPUTS {
        anyhow::bail!("Too many UTXOs to consolidate in one transaction ({} > {}). Wait for next update for multi-pass.", live_coins.len(), mirstat::core::MAX_CONSOLIDATE_INPUTS);
    }
    ...
    for denom in mirstat::core::decompose_value(out_val) {
        let salt: [u8; 32] = rand::random();
        outputs.push(mirstat::core::OutputData::Standard { address: new_mss_pk, value: denom, salt });
    }
    ...
    let mirstat::core::types::Witness::ScriptInputs(inputs) = witness;
    witnesses.push(inputs.iter().map(hex::encode).collect::<Vec<_>>().join(","));

    let reveal_req = rpc::SendTransactionRequest {
        inputs: input_reveals.iter().map(|ir| rpc::InputRevealJson {
            bytecode: match &ir.predicate { mirstat::core::types::Predicate::Script { bytecode } => hex::encode(bytecode) },
            value: ir.value,
...
async fn wallet_defrag(
    ...
    println!(
        ...
        mirstat::core::types::encode_address_with_checksum(&dest_addr)
    );
    ...
    if private {
        ...
        loop {
            ...
            if current_in_sum >= total_send + required_fee {
                ...
                for spec in &recipient_specs {
                    match spec {
                        ParsedOutput::Standard(address, value) => {
                            for denom in mirstat::core::decompose_value(*value) {
                                let salt: [u8; 32] = rand::random();
                                all_outputs.push(mirstat::core::OutputData::Standard { address: *address, value: denom, salt });
                            }
                        }
                        ParsedOutput::Stateful(address, state) => {
                            let salt: [u8; 32] = rand::random();
                            all_outputs.push(mirstat::core::OutputData::Confidential { address: *address, commitment: *state, salt });
                        }
                    }
                }
                ...
                if final_change > 0 {
                    for denom in mirstat::core::decompose_value(final_change) {
                        let seed = wallet.allocate_next_wots_seed()?;
                        let pk = mirstat::core::wots::keygen(&seed);
                        let addr = mirstat::core::compute_address(&pk);
                        let salt: [u8; 32] = rand::random();
                        let idx = all_outputs.len();
                        all_outputs.push(mirstat::core::OutputData::Standard { address: addr, value: denom, salt });
                        change_seeds.push((idx, seed));
                    }
                }
                ...
                // 3. Shuffle outputs to obfuscate which is the recipient vs change
                {
                    ...
                    let shuffled: Vec<mirstat::core::OutputData> = indices.iter().map(|&i| all_outputs[i].clone()).collect();
...
async fn submit_commit(
    ...
    let spam_nonce = tokio::task::spawn_blocking(move || {
        mirstat::core::transaction::mine_pow(&commitment_owned, required_pow, current_height, header_hash)
    }).await?;
...
async fn do_reveal(
    ...
    let reveal_req = rpc::SendTransactionRequest {
        ...
        signatures: signatures.iter().map(|s| match s {
            mirstat::core::types::Witness::ScriptInputs(inputs) => {
...
async fn wallet_mix(
    ...
    let register_req = rpc::MixRegisterRequest {
        ...
        input: rpc::InputRevealJson {
            bytecode: match &input.predicate { mirstat::core::types::Predicate::Script { bytecode } => hex::encode(bytecode) },
            value: input.value,
        ...
        match wallet.prepare_mix_fee(&live_coins) {
            Ok((fee_input, fee_cid)) => {
                let fee_req = rpc::MixFeeRequest {
                    mix_id: mix_id_hex.clone(),
                    input: rpc::InputRevealJson {
                        bytecode: match &fee_input.predicate { mirstat::core::types::Predicate::Script { bytecode } => hex::encode(bytecode) },
...
async fn wallet_automix(
    ...
    let list_resp: mirstat::rpc::MixListResponse = client.get(format!("{}/mix/list", base_url))
        .send().await?.json().await?;
    ...
    for session in list_resp.sessions {
        if session.phase == "collecting" {
            ...
            if required_value + cost + 1 <= coin_val {
                ...
                let pk1 = mirstat::core::wots::keygen(&seed1);
                let addr1 = mirstat::core::compute_address(&pk1);
                let salt1: [u8; 32] = rand::random();
                let idx1 = outputs.len();
                outputs.push(mirstat::core::OutputData::Standard { address: addr1, value: session.denomination, salt: salt1 });
                ...
                let pk2 = mirstat::core::wots::keygen(&seed2);
                let addr2 = mirstat::core::compute_address(&pk2);
                let salt2: [u8; 32] = rand::random();
                let idx2 = outputs.len();
                outputs.push(mirstat::core::OutputData::Standard { address: addr2, value: 1, salt: salt2 });
    ...
    if change_value > 0 {
        let change_denoms = mirstat::core::decompose_value(change_value);
    ...
    // Shuffle outputs to prevent sequential linkage on-chain
    {
        ...
        indices.shuffle(&mut rand::thread_rng());
        let shuffled_outputs: Vec<mirstat::core::OutputData> = indices.iter().map(|&i| outputs[i].clone()).collect();
...
fn wallet_history(
    ...
    let mut filtered: Vec<(usize, &mirstat::wallet::HistoryEntry)> = history
        .iter()
        .enumerate()
        .rev() 
        .collect();
...
fn wallet_import_rewards(path: &PathBuf, coinbase_file: &PathBuf, data_dir: &PathBuf) -> Result<()> {
    ...
    let storage = mirstat::storage::Storage::open(inferred_data_dir.join("db"))
    ...
    let new_coins: Vec<wallet::WalletCoin> = entries
        ...
        .map(|entry| {
            ...
            let seed = mirstat::wallet::coinbase_seed(&mining_seed, entry.height, entry.index);
            let salt = parse_hex32(&entry.salt).unwrap();
            let address = parse_hex32(&entry.address).unwrap();
            let owner_pk = mirstat::core::wots::keygen(&seed);
            let coin_id = mirstat::core::compute_coin_id(&address, entry.value, &salt);

            mirstat::wallet::WalletCoin {
                seed,
                owner_pk,
                address,
                value: entry.value,
    ...
    for wc in new_coins {
        if existing_coins.contains(&wc.coin_id) {
            ...
            wallet.data.history.push(mirstat::wallet::HistoryEntry {
                inputs: vec![],
...
async fn wallet_issue_license(
    ...
    let poaw_commitment_bytes: [u8; 32] = if let Some(bundle_path) = bundle {
        ...
        for (i, &h) in sampled.iter().enumerate() {
            ...
            let pow_hash = mirstat::core::types::hash(&data);
            if mirstat::core::types::count_leading_zeros(&pow_hash) < difficulty {
    ...
    let license_meta = mirstat::wallet::LicenseMetadata {
    ...
    let meta_hash = mirstat::core::types::hash(&meta_bytes);
    ...
    let salt = mirstat::core::types::hash(&salt_input);

    let covenant_script = mirstat::wallet::Wallet::build_pruning_license_covenant(license_meta.issuer, fixed_fee);
    let covenant_address = mirstat::compute_address(&mirstat::core::types::hash(&covenant_script));

    let license_output = mirstat::core::OutputData::Confidential {
    ...
    println!("   Fixed Royalty Fee: {} mirstat (paid on every transfer)", fixed_fee);
    ...
    let burn_output = mirstat::core::OutputData::DataBurn {
    ...
    let mut metadata_burns: Vec<mirstat::core::OutputData> = Vec::new();
    ...
    for (i, chunk) in meta_bytes.chunks(CHUNK_DATA_SIZE).enumerate() {
        ...
        metadata_burns.push(mirstat::core::OutputData::DataBurn {
            payload,
            value_burned: 0,
        });
    }
    ...
    if change > 0 {
        for denom in mirstat::core::decompose_value(change) {
            let seed = wallet.allocate_next_wots_seed()?;
            let pk = mirstat::core::wots::keygen(&seed);
            let addr = mirstat::core::compute_address(&pk);
            let idx = outputs.len();
            outputs.push(mirstat::core::OutputData::Standard { address: addr, value: denom, salt: rand::random() });
            change_seeds.push((idx, seed));
        }
    }
    ...
    println!("     mirstat wallet reveal --path {} --rpc-port {} --rpc-host {} --commitment {}",
    ...
    println!("  (Manual fallback: mirstat wallet rekey-license --path {} --old {} --new-coin-id <real-coin-id>)",
...
async fn wallet_buy_license(
    ...
    let onchain_commitment = mirstat::core::types::hash(&serde_json::to_vec(&meta)?);
    ...
    let meta_commitment = mirstat::core::types::hash(&serde_json::to_vec(&meta).unwrap_or_default());
    ...
    let secret_hash: [u8; 32] = if let Ok(h) = std::env::var("mirstat_HTLC_SECRET_HASH") {
        let mut bytes = [0u8; 32];
        hex::decode_to_slice(h, &mut bytes).context("Invalid mirstat_HTLC_SECRET_HASH")?;
        bytes
    } else {
        bail!("Buyer must provide seller-generated secret hash via mirstat_HTLC_SECRET_HASH env var (or --secret-hash in full CLI)");
    };
    ...
    let refund_pk = mirstat::core::wots::keygen(&refund_seed);
    ...
    let royalty_output = mirstat::core::OutputData::Standard {
        address: meta.issuer,
        value: transfer_fee,
        salt: rand::random(),
    };
    ...
    println!("   Transaction commitment: {}", hex::encode(tx_commitment));
    println!("   After mining, run `mirstat wallet reveal --commitment {}` to complete the transfer.", hex::encode(tx_commitment));
    println!();
...
async fn wallet_sell_license(
    ...
    if let Some(secret_str) = secret_hex {
        ...
        let secret_hash = mirstat::core::types::hash(&secret);
        ...
        let covenant_script = mirstat::wallet::Wallet::build_pruning_license_covenant(meta.issuer, transfer_fee);
        let covenant_address = mirstat::compute_address(&mirstat::core::types::hash(&covenant_script));
        ...
        let new_license_output = mirstat::core::OutputData::Confidential {
            address: covenant_address,
            commitment: meta_hash,
            salt,
        };
        ...
        // Burn
        outputs.push(mirstat::core::OutputData::DataBurn {
            payload: b"pruning-license-transfer".to_vec(),
            value_burned: burn_amount,
        });
        ...
        println!("   Submit with: mirstat wallet reveal --commitment {}", hex::encode(tx_commitment));
        println!();
        ...
        println!("  mirstat_HTLC_SECRET_HASH={} mirstat wallet buy-license ... --seller-pk YOUR_PK", hex::encode(secret_hash));
        println!();
...
async fn wallet_recover_license_metadata(
            ...
            if let Some(txs) = batch_json.get("transactions").and_then(|t| t.as_array()) {
                ...
                if !chunks.is_empty() {
                    ...
                    for (total, mut group) in by_total {
                        if group.len() == total as usize {
                            ...
                            if let Ok(meta) = serde_json::from_slice::<mirstat::wallet::LicenseMetadata>(&full) {
                                let meta_commitment = mirstat::core::types::hash(&full);
                                wallet.store_license_metadata(meta_commitment, meta.clone())?;
                                ...
                                println!("   IMPORTANT: Run `mirstat wallet rekey-license --path {} --old {} --new-coin-id <real-coin-id>` after the license appears on-chain.", path.display(), hex::encode(meta_commitment));
                                recovered_any = true;
                            }
                        }
                    }
                ...
                if !recovered_any {
                    for payload in raw_payloads {
                        if let Ok(meta) = serde_json::from_slice::<mirstat::wallet::LicenseMetadata>(&payload) {
                            let key = mirstat::core::types::hash(&payload);
    ...
    if !recovered_any {
        ...
        println!("  1. Use a block explorer or `mirstat` node logs to find the DataBurn output in the tx.");
...
println!("\n✓ MSS Address Generated!");
    ...
    println!("  Address:  {}", mirstat::core::types::encode_address_with_checksum(&root));
...
pub async fn run_node(
    ...
    if let Some(wp) = effective_license_wallet {
        match std::env::var("mirstat_LICENSE_WALLET_PASSWORD") {
            ...
            _ => {
                tracing::info!(
                    "license_wallet configured ({}) but mirstat_LICENSE_WALLET_PASSWORD env var not set; skipping auto-register of licenses",
                    wp.display()
                );
            }
        }
    ...
    if mine {
        let simd = mirstat::core::simd_mining::detected_level();
...
async fn commit_transaction(rpc_port: u16, rpc_host: String, coins: Vec<String>, destinations: Vec<String>) -> Result<()> {
    ...
    let salt: [u8; 32] = rand::random();
    let commitment = mirstat::core::compute_commitment(&input_coins, &dest_hashes, &salt);
...
async fn get_state(rpc_port: u16, rpc_host: String) -> Result<()> {
    ...
    println!("  mirstat:     {}", response.mirstat);
...
async fn keygen(rpc_port: Option<u16>, rpc_host: String) -> Result<()> {
    if let Some(port) = rpc_port {
        ...
        println!("  Address:  {}", mirstat::core::types::encode_address_with_checksum(&address));
    }
...
async fn sync_from_genesis(data_dir: PathBuf, peer_addr: String, port: u16) -> Result<()> {
    let storage = mirstat::storage::Storage::open(data_dir.join("db"))?;
    let syncer = mirstat::sync::Syncer::new(storage.clone());
    ...
    let mut network = mirstatNetwork::new(keypair, listen_addr, vec![peer_multiaddr], std::collections::HashSet::new()).await?;
    ...
    mirstat::sync::Syncer::verify_header_chain(&headers, &[])?;
    let our_state = storage.load_state()?.unwrap_or_else(|| mirstat::core::State::genesis().0);
    ...
    let window_start = fork_height.saturating_sub(mirstat::core::DIFFICULTY_LOOKBACK as u64);
    ...
    let mut state_before_prev: Option<mirstat::core::State> = None;
    ...
    while dl_cursor < peer_height {
        ...
        for batch in &batches {
                ...
                if e.to_string().contains("State root mismatch") && height > 0 && height < crate::core::types::COMMIT_REPLAY_FIX_ACTIVATION_HEIGHT {
                  //  tracing::warn!("State root mismatch at {}. Simulating historical node restart to self-heal...", height);
                    if let (Some(s_prev), Some(h_prev)) = (state_before_prev.clone(), headers_before_prev.clone()) {
                        ...
                        state.target = mirstat::core::state::adjust_difficulty(&state);
                        recent_headers.push(batch_prev.timestamp);
                        if recent_headers.len() > mirstat::core::MEDIAN_TIME_PAST_WINDOW { recent_headers.remove(0); }
            ...
            if recent_headers.len() > mirstat::core::MEDIAN_TIME_PAST_WINDOW { 
    ...
    println!("  mirstat:    {}", hex::encode(state.mirstat));
    Ok(())
}
