...
pub enum Predicate {
    /// A compiled mirstatScript bytecode payload.
...
impl Batch {
    ...
    pub fn header(&self) -> BatchHeader {
        let mut mirstat = self.prev_mirstat;

        // 1. Replay transaction commitments
        for tx in &self.transactions {
            match tx {
                Transaction::Commit { commitment, .. } => {
                    mirstat = hash_concat(&mirstat, commitment);
                }
                Transaction::Reveal { inputs, outputs, salt, .. } | Transaction::Consolidate { inputs, outputs, salt, .. } => {
                    ...
                    mirstat = hash_concat(&mirstat, &tx_hash);
                }
            }
        }
...
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct Batch {
    /// The mirstat of the previous batch this one extends
    pub prev_mirstat: [u8; 32],
...
pub fn compute_header_hash(header: &BatchHeader) -> [u8; 32] {
    ...
    hasher.update(&header.post_tx_mirstat);
...
pub const NETWORK_MAGIC: &[u8] = b"mirstat_MAINNET_V1";
...
mod tests {
    ...
    fn genesis_deterministic() {
        ...
        assert_eq!(s1.mirstat, s2.mirstat);
