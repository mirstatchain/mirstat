...
//
//   per nonce:  h = blake3_40(mirstat || nonce_le8)        // block_len = 40
...
struct Params {
    mirstat: array<u32, 8>,  // u32::from_le_bytes of each 4-byte group of the 32-byte mirstat
...
fn first_compress(gid: u32) -> array<u32,8> {
    var m: array<u32,16>;
    m[0] = P.mirstat[0]; m[1] = P.mirstat[1]; m[2] = P.mirstat[2]; m[3] = P.mirstat[3];
    m[4] = P.mirstat[4]; m[5] = P.mirstat[5]; m[6] = P.mirstat[6]; m[7] = P.mirstat[7];
    let n = nonce_for(gid);
    m[8] = n.x; m[9] = n.y;
...
type JobKey = ([u8; 32], [u8; 32], Option<[u8; 32]>); // (mirstat, target, pool_target)
...
impl GpuMiner {
    ...
    pub fn mine_gpu(
        ...
        let job: JobKey = (mirstat, target, pool_target);
        ...
        loop {
            ...
            for (nonce, _kind) in winners {
                let final_hash = create_extension(mirstat, nonce).final_hash;
    ...
    pub fn self_test(&self) -> Result<()> {
        let mirstat = [0xA5u8; 32]; // any fixed input; we compare GPU vs CPU on it
        ...
        for gid in 0..SELFTEST_N as u64 {
            let expected = super::extension::create_extension(mirstat, base + gid).final_hash;
            let mut got = [0u8; 32];
            for i in 0..8usize {
...
pub fn mine(
    ...
    if want_gpu {
        if let Some(g) = shared() {
            return g.mine_gpu(mirstat, target, pool_target, cancel, hash_counter);
        }
        if backend() == Backend::Gpu {
            tracing::warn!("GPU backend requested but no usable GPU; mining on CPU");
        }
    }
    mine_extension(mirstat, target, pool_target, threads, cancel, hash_counter)
}
