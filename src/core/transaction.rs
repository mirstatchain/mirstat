    ...
    match tx {
        ...
        Transaction::Reveal { inputs, witnesses, outputs, salt, .. } => {
...

            {
                ...
                state.mirstat = hash_concat(&state.mirstat, &tx_hash);
            }
        ...
        Transaction::Consolidate { inputs, witness: _, outputs, salt, .. } => {
            ...
            }
            {
                ...
                hasher.update(salt);
                state.mirstat = hash_concat(&state.mirstat, hasher.finalize().as_bytes());
            }
...
///       root(state.coins' ∪ state.commitments', v2) = declared mirstat update
///
///   result = Err(_)  ⇒
...
pub fn apply_transaction(state: &mut State, tx: &Transaction) -> Result<()> {
    ...
    match tx {
        Transaction::Commit { commitment, spam_nonce } => {
            ...
            state.mirstat = hash_concat(&state.mirstat, commitment);
            Ok(())
        }
        ...
        Transaction::Reveal { inputs, witnesses, outputs, salt, .. } => {
            ...
            // 8. Update mirstat (SegWit: witnesses are explicitly excluded.
...
/// Execute a Witness against a Predicate via the mirstatScript VM.
fn verify_predicate(
    predicate: &Predicate,
    witness: &Witness,
    commitment: &[u8; 32],
