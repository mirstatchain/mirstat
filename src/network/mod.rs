...
pub use protocol::{Message, mirstatCodec, mirstat_PROTOCOL, MAX_GETBATCHES_COUNT, MAX_GETHEADERS_COUNT};
...
pub struct mirstatBehaviour {
    pub rr: request_response::Behaviour<mirstatCodec>,
...
pub struct mirstatNetwork {
    swarm: Swarm<mirstatBehaviour>,
...
.with_other_transport(|keypair| {
            ...
            // --- decrease to 60s to PREVENT SYNC TIMEOUTS
                ...
                let rr = request_response::Behaviour::new(
                    [(mirstat_PROTOCOL, ProtocolSupport::Full)],
                    rr_config,
                );
                // -------------------------------------------------
    ...
    pub async fn next_event(&mut self) -> NetworkEvent {
        loop {
            tokio::select! {
                ...
                // ── Request-Response ────────────────────────────────
                SwarmEvent::Behaviour(mirstatBehaviourEvent::Rr(
                ...
                // ── Identify ────────────────────────────────────────
                SwarmEvent::Behaviour(mirstatBehaviourEvent::Identify(
                ...
                SwarmEvent::Behaviour(mirstatBehaviourEvent::Light(_)) => {}


                // ── AutoNAT ─────────────────────────────────────────
                SwarmEvent::Behaviour(mirstatBehaviourEvent::Autonat(
                ...
                // ── Relay client ────────────────────────────────────
                SwarmEvent::Behaviour(mirstatBehaviourEvent::RelayClient(
                ...
                SwarmEvent::Behaviour(mirstatBehaviourEvent::RelayClient(event)) => {
                    tracing::debug!("Relay client event: {:?}", event);
                }

                // ── DCUtR (hole-punch) ──────────────────────────────
                SwarmEvent::Behaviour(mirstatBehaviourEvent::Dcutr(event)) => {
                    tracing::info!("DCUtR event: {:?}", event);
                }

                // ── Relay server ────────────────────────────────────
                SwarmEvent::Behaviour(mirstatBehaviourEvent::RelayServer(event)) => {
                    tracing::debug!("Relay server event: {:?}", event);
                }

                // ── Kademlia ────────────────────────────────────────
                SwarmEvent::Behaviour(mirstatBehaviourEvent::Kademlia(
                    kad::Event::RoutingUpdated { peer, .. },
                )) => {
                    tracing::debug!("Kademlia routing updated: {}", peer);
                }
                SwarmEvent::Behaviour(mirstatBehaviourEvent::Kademlia(_)) => {}
...
mod tests {
    ...
    fn nat_status_default() {
        // Can't construct mirstatNetwork without async, but we can
