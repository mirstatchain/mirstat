/**
 * @fileoverview mirstat Web Wallet — Worker Thread
 *
...
// ── Contract tracking (mirstatConnect) ─────────────────────────────────────
 ...
 * miner workers need: mirstat, target, and the full batch to submit.
 *
...
async function handleGetTemplate() {
    ...
    try {
        ...
        if (miningMode === 'pool') {
            ...
            return {
                mining_mirstat: tmpl.mining_mirstat,
                target:          tmpl.target,
        ...
        return {
            mining_mirstat: template.mining_mirstat,
...
async function handleSubmitMinedBlock(template, nonce) {
    ...
    isSubmitting = true;
    try {
        const extStr = wallet.build_solo_extension(template.mining_mirstat, BigInt(nonce));
...
self.onmessage = async (e) => {
    ...
    try {
        if (type === 'INIT') {
            ...
            // secret hash. The maker generates the secret (see DEX_LOCK_mirstat).
            ...
            try {
                ...
                self.postMessage({ type: 'DEX_mirstat_LOCKED_SUCCESS', payload: {
                    offerId,
            ...
            // taker to lock ETH against a fake or short mirstat lock.
            ...
            try {
                ...
                // SAFETY: the mirstat HTLC timeout is ABSOLUTE (fixed when the maker
                // locked), but our Base refund is RELATIVE to when we lock. If the
                // mirstat leg expires too soon, a malicious maker could refund MDS
                // AND claim our ETH. Require the remaining mirstat window to exceed
                ...
                if (remaining < minRemaining) {
                    ok = false;
                    reason = `mirstat HTLC expires in ~${remaining} blocks; need ≥ ${minRemaining}. Unsafe to lock ETH.`;
                }
        ...
        else if (type === 'DEX_CLAIM_mirstat') {
            await acquireSendLock();
            try {
            ...
            // DEX_LOCK_mirstat on-chain lock.
            //
            ...
            // Tie the L2 HTLC timeout under the Base refund window. htlcTimeout is a mirstat
        ...
        // ═══════════ ESCROWED-BID FILLS (Part 2) ═══════════
            ...
            // branch of DEX_LOCK_mirstat, but the hashlock comes from the BID (the
        ...
        // ═══════════════════════════════════════════════════════════════════════════
            ...
            try {
                ...
                for (let i = 0; i < mssKeys.length; i++) {
                    ...
                    // imports the identical mirstat::wallet::hd::derive_mss_seed. For HD
        ...
        for (const tx of block.transactions) {
            ...
            if (action.inputs) {
                for (const inp of action.inputs) {
                    ...
                    // ── Remove spent contract coins (mirstatConnect) ──
                    {
            ...
            if (action.outputs) {
                for (const out of action.outputs) {
                    ...
                    // ── Contract coins at a watched address (mirstatConnect) ──
                    {
...
// ═══════════════════════════════════════════════════════════════════════════════
//  Contract Funding & Execution (mirstatConnect)
// ═══════════════════════════════════════════════════════════════════════════════
