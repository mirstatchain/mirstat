/**
 * mirstat-provider.js — "mirstatConnect" dApp provider
 * =======================================================
 *
 * The web-native equivalent of `window.ethereum`. A dApp (e.g. the mirstat IDE,
 ...
 *
 *   <script src="mirstat-provider.js"></script>
 *   <script>
 *     mirstat.configure({ walletUrl: new URL("wallet/", location.href).href });
 *     const { address } = await mirstat.connect();
 *     const { txid }    = await mirstat.fundContract({ contractAddress, amount: 100_000 });
 *     const { txid }    = await mirstat.spendContract({ bytecode, inputs, outputs },
...
(function (global) {
  ...
  var PROTOCOL = "mirstat-bridge";
  var VERSION = 1;

  var config = {
    // Where the wallet is served. Defaults to "<this origin>/wallet/", which is
    // correct when the dApp and wallet share an origin (the mirstat convention).
    // Override via mirstat.configure({ walletUrl }) for a cross-origin wallet.
  ...
  function openCentredPopup(url) {
    ...
    return global.open(url, "mirstatWallet", features);
  }
  ...
  function invoke(action, params, opts) {
    ...
    if (!walletOrigin) {
      return Promise.reject(new Error("mirstat: walletUrl/walletOrigin is not configured."));
    }
    ...
    if (!popup) {
      return Promise.reject(new Error(
        "mirstat: the wallet popup was blocked. Trigger connect/fund/spend directly " +
    ...
    return new Promise(function (resolve, reject) {
      ...
      function armHandshakeTimer() {
        ...
        handshakeTimer = setTimeout(function () {
          finish(new Error("mirstat: wallet did not respond (handshake/idle timeout)."));
      ...
      function onMessage(event) {
        ...
        switch (d.type) {
          ...
          case "error":
            finish(new Error(d.message || "mirstat: wallet returned an error."));
            break;
      ...
      closedPoll = setInterval(function () {
        if (popup.closed) finish(new Error("mirstat: the user closed the wallet."));
      }, 400);

      // Optional hard overall cap.
      if (opts.resultTimeoutMs) {
        setTimeout(function () {
          finish(new Error("mirstat: request timed out."));
  ...
  var mirstat = {
    ...
    fundContract: function (p, opts) {
      if (!p || !p.contractAddress) {
        return Promise.reject(new Error("mirstat.fundContract: contractAddress is required."));
      }
      return invoke("fund", {
    ...
    spendContract: function (p, opts) {
      if (!p || !p.bytecode) {
        return Promise.reject(new Error("mirstat.spendContract: bytecode is required."));
      }
  ...
  global.mirstat = mirstat;
  // ...and support module consumers without breaking the global.
  if (typeof module !== "undefined" && module.exports) module.exports = mirstat;
