...
//
// Stateless: receives a template (mirstat + target), searches nonces
...
self.onmessage = async (e) => {
    ...
    if (type === 'INIT') {
        ...
        mining = true;
        loopRunning = true;
        const { mirstat, target } = payload;
        if (payload.throttle_ms !== undefined) throttleMs = payload.throttle_ms;

        self.postMessage({ type: 'LOG', payload: `START received. mirstat=${mirstat?.substring(0,16)}… throttle=${throttleMs}ms` });

        if (!mirstat || !target) {
            self.postMessage({ type: 'ERROR', payload: 'START missing mirstat or target' });
            mining = false;
            loopRunning = false;
            return;
        }
...
// Each worker starts at a random nonce offset to avoid overlap.
        ...
        try {
            while (mining) {
                const result = search_nonces(mirstat, target, nonce, CHUNK);
