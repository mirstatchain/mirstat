//! # wasm-wallet — WebAssembly wallet for the mirstat blockchain
//!
...
use mirstat::core::mss::MssKeypair;
use mirstat::core::types::{compute_address, compute_commitment, compute_coin_id, decompose_value};
use mirstat::wallet::hd::{generate_mnemonic, master_seed_from_mnemonic, derive_wots_seed, derive_mss_seed};
...
const SALT_DOMAIN: &[u8] = b"mirstat/salt/v1";
...
///
/// The mirstat UTXO model requires all coin values to be exact powers of 2.
...
pub fn mine_commitment_pow(commitment_hex: &str, required_pow: u32, target_height: u64, header_hash_hex: &str) -> u64 {
    ...
    mirstat::core::transaction::mine_pow(&commitment, required_pow, target_height, header_hash)
}
...
/// # Arguments
///
/// * `mirstat_hex` — 64-char hex of the block header mirstat.
...
#[wasm_bindgen]
pub fn search_nonces(mirstat_hex: &str, target_hex: &str, start_nonce: u64, iterations: u32) -> Option<u64> {
    let mut mirstat = [0u8; 32];
    if hex::decode_to_slice(mirstat_hex, &mut mirstat).is_err() {
        return None;
    }
    ...
    for i in 0..iterations {
        ...
        let results = mirstat::core::simd_mining::create_extensions_4way(mirstat, nonces);
...
pub fn decrypt_cli_wallet(data: &[u8], password: &str) -> Result<String, JsValue> {
    let decrypted = mirstat::wallet::crypto::decrypt(data, password.as_bytes())
...
/// Safely parses a checksummed hex-encoded mirstat address into a 32-byte array.
///
...
fn parse_address_wasm(s: &str) -> Result<[u8; 32], JsValue> {
    ...
    let actual_checksum_hash = mirstat::core::types::hash(&addr);
...
pub fn address_to_checksummed_hex(address_hex: &str) -> Result<String, JsValue> {
    ...
    Ok(mirstat::core::types::encode_address_with_checksum(&addr))
}
...
impl WebWallet {
    ...
    pub fn build_state_thread_tx(
        ...
        let contract_addr = mirstat::core::types::hash(&contract_bytes);
        ...
        for inp in &selected {
            ...
            let bytecode = mirstat::core::script::compile_p2pk(&pk);
            input_reveals.push(serde_json::json!({
    ...
    /// # Returns
    ///
    /// `None` if `mirstat_hex` is not valid 64-character hex.
#[wasm_bindgen]
pub fn build_solo_extension(&self, mirstat_hex: &str, nonce: u64) -> Option<String> {
    let mut mirstat = [0u8; 32];
    hex::decode_to_slice(mirstat_hex, &mut mirstat).ok()?;

    let ext = mirstat::core::extension::create_extension(mirstat, nonce);
    ...
    pub fn get_mss_address(&mut self, index: u32, height: u32, progress_cb: Option<js_sys::Function>) -> Result<String, JsValue> {
        ...
        let kp = mirstat::core::mss::keygen_with_progress(&seed, height, |current, total| {
    ...
    pub fn check_filter(&self, filter_hex: &str, block_hash_hex: &str, n: u32) -> bool {
        ...
        mirstat::core::filter::match_any(&filter_data, &block_hash, n as u64, &self.watchlist)
    }
...
/// Plans a defragmentation batch: moves up to `max_inputs` fragmented
    ...
    pub fn prepare_defrag(
        ...
        let commitment = mirstat::core::types::compute_commitment(&input_coin_ids, &output_hashes, &tx_salt);
    ...
    pub fn prepare_script_spend(
        ...
        loop {
            ...
            let mss_sig_len: u64 = (8 + 32 + wots::SIG_SIZE + 4
                + (mirstat::core::mss::DEFAULT_HEIGHT as usize) * 32) as u64;
        ...
        for inp in &selected {
            ...
            let p2pk_addr = mirstat::core::types::hash(&mirstat::core::script::compile_p2pk(&pk));
            let mut salt_b = [0u8; 32];
        ...
        if contract_inputs.len() + wallet_inputs.len() > mirstat::core::types::MAX_TX_INPUTS {
            return Err(JsValue::from_str("Too many inputs for one transaction (max 256)"));
        }
        if outputs_out.len() > mirstat::core::types::MAX_TX_OUTPUTS {
            return Err(JsValue::from_str(
...
// ── Funding a contract ──────────────────────────────────────────────────
    ...
    pub fn prepare_fund_tx(
        ...
        for inp in &selected {
            ...
            let p2pk = mirstat::core::types::hash(&mirstat::core::script::compile_p2pk(&pk));
            let mut salt = [0u8; 32];
        ...
        if wallet_inputs.len() > mirstat::core::types::MAX_TX_INPUTS {
    ...
    pub fn prepare_fund_many(
        ...
        if let Some(ref burns_str) = databurns_json {
            ...
            for b in &burns {
                ...
                if payload.len() > mirstat::core::MAX_BURN_DATA_SIZE {
                    return Err(JsValue::from_str(&format!(
                        "DataBurn payload {}B exceeds node max {}B",
                        payload.len(), mirstat::core::MAX_BURN_DATA_SIZE)));
                }
...
pub fn blake3_hash_hex(hex_data: &str) -> String {
    ...
    let h = mirstat::core::types::hash(&bytes);
    hex::encode(h)
}
...
pub fn compute_commitment_hex(input_ids_json: &str, output_hashes_json: &str, salt_hex: &str) -> Result<String, JsValue> {
    ...
    let commitment = mirstat::core::types::compute_commitment(&in_bytes, &out_bytes, &salt);
...
pub fn mine_chat_pow_v2_wasm(
    ...
    let attachments: Vec<mirstat::chat::ChatAttachment> = serde_json::from_str(attachments_json)
        .map_err(|e| JsValue::from_str(&format!("Invalid attachments JSON: {}", e)))?;

    // The Rust miner function handles the heavy lifting
    let nonce = mirstat::chat::mine_chat_pow_v2(
        sender.to_string(),
        timestamp,
        reply_to,
        words,
...
pub fn build_channel_state(
    ...
    for (i, h) in htlcs.into_iter().enumerate() {
        ...
        let script = mirstat::core::script::compile_htlc(&secret_hash, receiver_pk, h.timeout, refund_pk);
        let htlc_addr = mirstat::core::types::hash(&script);
    ...
    let mut hasher = blake3::Hasher::new();
    hasher.update(mirstat::core::types::NETWORK_MAGIC);
...
pub fn build_multisig_2of2_address(pk1_hex: &str, pk2_hex: &str) -> String {
    ...
    let script = mirstat::core::script::compile_multisig_2of2(&pk1, &pk2);
...
pub fn qbolt_channel_address(
    ...
    Ok(hex::encode(mirstat::core::types::hash(&script)))
}
...
fn qb_build_state_core(
    ...
    for (i, h) in htlcs.iter().enumerate() {
        ...
        let script = mirstat::core::script::compile_htlc(&secret_hash, receiver_pk, h.timeout, sender_pk);
        let htlc_addr = mirstat::core::types::hash(&script);
        for (j, denom) in decompose_value(h.amount).into_iter().enumerate() {
...
pub fn qbolt_build_state(
    ...
    let s = qb_hex32(sender_pk_hex, "sender pk")?;
    let r = qb_hex32(receiver_pk_hex, "receiver pk")?;
    let addr = mirstat::core::types::hash(&qbolt_covenant_bytes(&s, &r, expiry));
...
fn qb_build_reveal_core(
    ...
    let addr = mirstat::core::types::hash(channel_addr_script);
    let funding = qb_sorted_funding(funding_json, &addr)?;
...
pub fn qbolt_build_legacy_close_state(
    ...
    let script = mirstat::core::script::compile_multisig_2of2(&a, &b);
    let addr = mirstat::core::types::hash(&script);
...
pub fn verify_mss_sig_wasm(sig_hex: &str, msg_hex: &str, pk_hex: &str) -> bool {
    ...
    if let Ok(sig) = mirstat::core::mss::MssSignature::from_bytes(&sig_bytes) {
        mirstat::core::mss::verify(&sig, &msg, &pk)
    } else {
        false
    }
}
...
pub fn compute_p2pk_address_hex(owner_pk_hex: &str) -> Result<String, JsValue> {
    ...
    Ok(hex::encode(mirstat::core::types::compute_address(&pk)))
}
...
pub fn build_covenant_htlc_bytecode_hex(
    ...
    let bytecode = mirstat::core::script::compile_covenant_htlc(
...
/// `mirstat::core::script::compile_limit_order_covenant` for the security notes.
#[wasm_bindgen]
pub fn build_limit_order_covenant_bytecode_hex(
    ...
    hex::decode_to_slice(refund_pk_hex, &mut refpk)
        .map_err(|_| JsValue::from_str("Invalid refund_pk"))?;
    let bytecode = mirstat::core::script::compile_limit_order_covenant(
        &sh, max_claim, timeout_height, &refpk,
    );
    Ok(hex::encode(bytecode))
}

/// Builds the mirstat HTLC bytecode for cross-chain atomic swaps.
#[wasm_bindgen]
pub fn build_htlc_bytecode_hex(
    ...
    let bytecode = mirstat::core::script::compile_htlc(&sh, &rpk, timeout_height, &refpk);
    Ok(hex::encode(bytecode))
}
...
mod tests {
    use super::*;
    use mirstat::core::types::hash;
    use mirstat::core::mss;
    ...
    fn parse_address_wasm_checksum_72_valid() {
        ...
        let checksum = mirstat::core::types::hash(&addr_bytes);
