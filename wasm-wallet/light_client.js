// light_client.js — Browser-side libp2p light client for mirstat wallet
//
// Connects to full nodes over WebRTC direct (no HTTPS, no domain, no cert authority).
// Speaks the /mirstat/light/2.0.0 JSON protocol over libp2p streams.
//
// Usage in worker.js:
...
onPushEvent(cb) {
    ...
    async start(addrs) {
        ...
        * 
         * ## 1. Reasoning
         * Listens for asynchronous Push streams initiated by the mirstat full node 
         ...
         * ```text
         * Pre:
         *   - Peer opens a new stream with protocol '/mirstat/light-push/2.0.0'
         ...
         */
        this.node.handle('/mirstat/light-push/2.0.0', async (stream, connection) => {
...
/**
     ...
     * mirstat Light Protocol. Previous iterations attempted to manually parse WebRTC 
     ...
     *   enforced by the mirstat rust node (`LightRequest` / `LightResponse` serialization).
     */
    /**
 * Outbound RPC over /mirstat/light/2.0.0 (libp2p v3 MessageStream API).
