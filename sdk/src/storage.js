...
export class NodeFSStorage extends StorageProvider {
    constructor(dirPath = './mirstat_wallet_data') {
        super();
...
export class BrowserStorage extends StorageProvider {
    async saveMetadata(dataStr) { localStorage.setItem('mirstat_wallet_meta', dataStr); }
    async loadMetadata() { return localStorage.getItem('mirstat_wallet_meta'); }
    async _openIDB() {
        return new Promise((resolve, reject) => {
            const req = indexedDB.open('mirstat_sdk_db', 1);
            req.onupgradeneeded = () => {
