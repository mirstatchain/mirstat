// test.js — Comprehensive mirstat SDK smoke test
//
// Coverage:
...
//                        GATED behind mirstat_LIVE_TX=1 because they broadcast
...
//
// Env flags:
//   mirstat_P2P_DEBUG=1     verbose stream tracing (handled inside p2p.js)
//   mirstat_LIVE_TX=1       enable network-mutating writes (chat + tx)
//   mirstat_WALLET_DIR=...  persistent wallet dir for the live tx (default ./my_bot_wallet)
//   mirstat_PUSH_WAIT_MS=N  how long to listen for push events (default 20000)
//   mirstat_PEER=<multiaddr> override the bootstrap peer
...
// ── Config ───────────────────────────────────────────────────────────────────
const PEER = process.env.mirstat_PEER
    || '/ip4/134.199.148.215/tcp/9333/p2p/12D3KooWPbR63SQg1UBLpAMiNngqrRHGM4LaMP8ieAJUxhfw7dxv';
const LIVE_TX = process.env.mirstat_LIVE_TX === '1';
const PUSH_WAIT_MS = Number(process.env.mirstat_PUSH_WAIT_MS || 20_000);
const TX_WALLET_DIR = process.env.mirstat_WALLET_DIR || './my_bot_wallet';
const SCAN_WINDOW = Number(process.env.mirstat_SCAN_WINDOW || 300);
const SCAN_PACE_MS = Number(process.env.mirstat_SCAN_PACE_MS || 550);
...
async function run() {
    ...
    try {
        ...
        const walletDir = await fs.mkdtemp(path.join(os.tmpdir(), 'mirstat-wallet-'));
        ...
        // ── Utils ──
        await step('mirstatUtils.formatMDS', () => {
            assert(mirstatUtils.formatMDS(0).prefix === 'MDS', '0 → MDS');
            assert(mirstatUtils.formatMDS(5000).prefix === 'kMDS', '5000 → kMDS');
            assert(mirstatUtils.formatMDS(2_000_000).prefix === 'mMDS', '2M → mMDS');
            assert(mirstatUtils.formatMDS(3_000_000_000).prefix === 'gMDS', '3G → gMDS');
            return 'MDS/kMDS/mMDS/gMDS boundaries ok';
        });

        await step('mirstatUtils.computeCoinId', () => {
            const id = mirstatUtils.computeCoinId(wotsAddr, 1000, randHex(32));
            assert(isHex(id, 64), `coin id not 64-hex: ${id}`);
            return id.slice(0, 16) + '…';
        });

        await step('mirstatUtils.hash (blake3)', () => {
            const h = mirstatUtils.hash(wotsAddr);
            assert(isHex(h, 64), `hash not 64-hex: ${h}`);
            // Deterministic.
            assert(mirstatUtils.hash(wotsAddr) === h, 'hash not deterministic');
            return h.slice(0, 16) + '…';
        });
        ...
        await step('Storage.NodeFSStorage round-trip', async () => {
            const sdir = await fs.mkdtemp(path.join(os.tmpdir(), 'mirstat-store-'));
            tempDirs.push(sdir);
            ...
            const wdir = await fs.mkdtemp(path.join(os.tmpdir(), 'mirstat-bal-'));
            tempDirs.push(wdir);
        ...
        // PEX wire format (offline). The binary /mirstat/2.0.0 protocol is bincode
        ...
        if (connected) {
            ...
            // ═══════════════════════════════════════════════════════════════
            if (LIVE_TX) {
                console.log('  (mirstat_LIVE_TX=1 — these broadcast to the real network)');
                ...
                skip('WRITE send_chat', 'set mirstat_LIVE_TX=1 to broadcast a real chat');
                skip('WRITE commit→send', 'set mirstat_LIVE_TX=1 (and fund a wallet) to broadcast a real tx');
            }
    ...
    if (fail > 0) {
        ...
        if (!LIVE_TX) console.log('   (Network writes were skipped — run with mirstat_LIVE_TX=1 to include them.)');
    }
}
